...
namespace CountryTagExtractor
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //"Акция началась около 18 часов. Около 20.00 появились первые задержанные: четыре человека, одного парня завели в автозак прямо с велосипедом. Задержана съемочная группа «Радыё Свабода» — журналистка Александра Дынько и оператор Андрей Рябчик. Их задержали, когда они вели стрим. Журналистов отпустили около 22 часов. Момент задержания журналистов «Радыё Свабода». Фото: Вадим Замировский, TUT.BY Кроме того, задержан журналист Reuters Владимир Костин. По словам нашего корреспондента, у него была аккредитация. Силовики сказали, что его увезли в Первомайское РУВД. Среди задержанных — журналист «Белсата» Ирина Ореховская. Ранее замминистра внутренних дел Александр  заявил, что журналистов задерживать не будут. Около 20.30 задержали еще одного человека, он стоял у филармонии с перевязанными руками. Что это было — непонятно. В задержали двоих участников акции и журналистку TUT.BY  Василевскую (спустя пару часов ее отпустили). В задержан фотограф TUT.BY Вадим Замировский. Задерживают людей возле парка . Правозащитники также сообщают о задержаниях журналистов в . Правозащитники составили список задержанных на сегодняшних акциях в городах ";
    ...
    static class CountryTagExtractor
    {
        ...
        private static string CreatePattern(string word)
        {
            string pattern = "";
            word = word.Replace('ё', 'е').Replace('й', 'и').Replace('Ё', 'Е').Replace('Й', 'И');
            string[] wordsStems = word.Contains(" ") ? word.Split(" ") : word.Contains("-") ? word.Split("-") : new string[] { word };
